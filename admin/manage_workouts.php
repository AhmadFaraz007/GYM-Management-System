...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_plan'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO workout_plans (title, description, created_by) VALUES (?, ?, ?)");
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $pdo->prepare("DELETE FROM workout_plans WHERE plan_id = ?")->execute([$id]);
    header("Location: manage_workouts.php");
    exit;
}

// Fetch plans
$plans = $pdo->query("SELECT wp.*, u.full_name AS creator FROM workout_plans wp LEFT JOIN users u ON wp.created_by = u.user_id ORDER BY wp.created_at DESC")->fetchAll();
?>
...
<head>
    ...
    <title>Manage Workouts | FlexFusion</title>
    <!-- Bootstrap CSS -->
...
<div class="sidebar">
    ...
    <ul class="sidebar-menu">
        ...
        <li><a href="manage_workouts.php" class="active"><i class="fas fa-dumbbell"></i> Manage Workouts</a></li>
...
<div class="main-content">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="mb-0">Manage Workout Plans</h2>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addWorkoutModal">
                <i class="fas fa-plus me-2"></i>Add Workout Plan
            </button>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-4">
                <div class="card workout-card fade-in">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><?= htmlspecialchars($plan['title']) ?></h5>
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#editWorkoutModal<?= $plan['plan_id'] ?>">
                                        <i class="fas fa-edit me-2"></i>Edit
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item text-danger" href="?delete=<?= $plan['plan_id'] ?>" 
                    ...
                    <div class="card-body">
                        ...
                        <div class="workout-meta">
                            <small class="text-muted">
                                <i class="fas fa-user me-1"></i>Created by: <?= $plan['creator'] ?? 'Unknown' ?>
                            </small>
                            <small class="text-muted d-block mt-2">
                                <i class="fas fa-clock me-1"></i>Created: <?= date('M d, Y', strtotime($plan['created_at'])) ?>
                            </small>
                        </div>
                    </div>
                </div>
        ...
        <div class="text-center py-5">
            <div class="empty-state">
                ...
                <button class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#addWorkoutModal">
                    <i class="fas fa-plus me-2"></i>Add Your First Workout Plan
                </button>
            </div>
        </div>
        <?php endif; ?>
